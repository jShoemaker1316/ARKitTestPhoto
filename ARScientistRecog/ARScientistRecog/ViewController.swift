...
class ViewController: UIViewController, ARSCNViewDelegate {
 ...
 //large UIFont and scale down to avoid issues
        let titleNode = textNode(scientist.name, font: UIFont.boldSystemFont(ofSize: 10))
        titleNode.pivotOnTopLeft()
 //add spacing to keep it from butting up against image
 ...
 //match size of flag to width of image of scientist seen
...
//make a method for our text boxes
    ...
    //sizing font to be way smaller, perfect sizing
        textNode.scale = SCNVector3(0.002, 0.002, 0.002)
        
        return textNode
    }
}
//AR extension to use every single time
extension SCNNode {
    var width: Float {
